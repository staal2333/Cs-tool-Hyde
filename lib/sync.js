...
export async function runSync() {
  ...
  for (const [company, h] of Object.entries(hist)) {
    ...
    if (fromConvo) {
      ...
      meta[company].person = fromConvo; // sync-derived; refresh each run (user edits live in the overlay)
    }
    ...
    // Status from the Gmail thread, but UPGRADE-ONLY so an incomplete or lagging
    // Gmail view can never undo what you (or a faster signal) already recorded:
    //  - last event is a real reply        → "Svar modtaget" (ball in our court)
    //  - 2+ mails sent, no reply after them → "Opfølgning sendt"
    //  - 1 mail sent                        → "Sendt"
    // Never downgrades (e.g. Opfølgning sendt → Sendt because the 2nd mail isn't
    // indexed yet). Booket / Nej tak are your decisions and never touched. The one
    // allowed step "back" is clearing "Svar modtaget" once your reply is the latest mail.
    const RANK = { 'Ikke kontaktet': 0, 'Sendt': 1, 'Opfølgning sendt': 2, 'Svar modtaget': 3 };
    const cur = (state[company] || {}).status || 'Ikke kontaktet';
    const lastEv = events[events.length - 1];
    let desired = null;
    if (lastEv && lastEv.dir === 'in' && !lastEv.auto) desired = 'Svar modtaget';
    else if (outCount >= 2) desired = 'Opfølgning sendt';
    else if (outCount >= 1) desired = 'Sendt';

    let apply = false;
    if (desired && cur !== 'Booket' && cur !== 'Nej tak' && desired !== cur) {
      if ((RANK[desired] ?? 0) > (RANK[cur] ?? 0)) apply = true;                       // forward only
      else if (cur === 'Svar modtaget' && lastEv && lastEv.dir === 'out') apply = true; // we answered → clear the flag
    }
    if (apply) {
      const date = fmtDate(lastEv ? lastEv.ts : Date.now());
      await applyPatch(company, { status: desired, date });
      state[company] = { ...(state[company] || {}), status: desired, date };
      if (desired === 'Sendt') summary.sent++;
      summary.updates.push(`${company} → ${desired}`);
    }
  }
