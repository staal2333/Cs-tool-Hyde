...
import { searchMessages, getMessage, fetchMessages, parseFrom, parseRecipients } from './gmail.js';
...
export async function runSync() {
  ...
  // Metadata-only + parallel fetch keeps us well under the function time limit.
  ...
  const toLog = []; // real replies (unseen) needing a full-body fetch for the log
  ...
  // Fetch full bodies only for the (few) unseen real replies, then log them.
  const bodies = await fetchMessages(token, toLog.map((t) => t.id), 'full');
  const bodyById = {};
  for (const b of bodies) bodyById[b.id] = b.body;
  for (const t of toLog) {
    const text = cleanReply(bodyById[t.id] || t.snippet);
