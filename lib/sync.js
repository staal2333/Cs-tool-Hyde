...
export async function runSync() {
  ...
  for (const [company, h] of Object.entries(hist)) {
    ...
    // Status follows the real Gmail thread so the pipeline can't drift:
    //  - last event is a real reply        → "Svar modtaget" (ball in our court)
    //  - 2+ mails sent, no reply after them → "Opfølgning sendt"
    //  - 1 mail sent                        → "Sendt"
    // The terminal decisions (Booket / Nej tak) are manual and never overridden.
    ...
    if (desired && cur !== 'Booket' && cur !== 'Nej tak' && cur !== desired) {
