...
import { searchMessages, fetchMessages, parseFrom, parseRecipients } from './gmail.js';
...
export async function runSync() {
  ...
  for (const m of inMsgs) {
    ...
    e.events.push({ dir: 'in', ts: m.date, date: fmtDate(m.date), subject, snippet: m.snippet || '', from, auto, threadId: m.threadId });
    if (!auto && m.date > e.lastInboundTs) { e.lastInboundTs = m.date; e.threadId = m.threadId || e.threadId; }
