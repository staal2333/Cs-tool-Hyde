...
export async function effectiveContacts() {
  ...
  const apply = (c) => {
    ...
    // Person name discovered in the Gmail conversation fills an empty contact.
    if (!out.person && meta[c.company]?.person) out = { ...out, person: meta[c.company].person };
    return out;
  };
  const list = [];
...
const FIELDS = ['temp', 'segment', 'placement', 'email', 'person', 'title', 'phone', 'subject', 'body', 'fuSubject', 'fuBody', 'crmStatus', 'buyer', 'buyerReason', 'tags', 'moneyScore', 'moneyReason', 'companyGuess'];
function clean(obj) {
  const out = {};
...
// Apply a {company: {buyer, buyerReason}} map in one write (used by auto-classify).
export async function bulkSetBuyer(map) {
  const o = await loadOverlay();
  o.edits = o.edits || {};
  for (const [company, val] of Object.entries(map)) {
    const patch = typeof val === 'string' ? { buyer: val } : { buyer: val.buyer, buyerReason: val.buyerReason || '' };
    const i = (o.custom || []).findIndex((c) => c.company === company);
    if (i >= 0) Object.assign(o.custom[i], patch);
    else o.edits[company] = { ...(o.edits[company] || {}), ...patch };
  }
  await saveOverlay(o);
}

// Apply enriched contact fields in one write. Each patch may carry
// person/title/phone/email (facts) and companyGuess (a marked AI guess).
export async function bulkEnrich(map) {
  const o = await loadOverlay();
  o.edits = o.edits || {};
  for (const [company, patch] of Object.entries(map)) {
    const clean = {};
    for (const k of ['person', 'title', 'phone', 'email', 'companyGuess']) {
      if (patch[k] !== undefined && patch[k] !== null && patch[k] !== '') clean[k] = patch[k];
    }
    if (!Object.keys(clean).length) continue;
    const i = (o.custom || []).findIndex((c) => c.company === company);
    if (i >= 0) Object.assign(o.custom[i], clean);
    else o.edits[company] = { ...(o.edits[company] || {}), ...clean };
  }
  await saveOverlay(o);
}

// Apply a {company: {moneyScore, moneyReason}} map in one write (budget rating).
export async function bulkSetMoney(map) {
  const o = await loadOverlay();
  o.edits = o.edits || {};
  for (const [company, val] of Object.entries(map)) {
    const patch = { moneyScore: val.moneyScore, moneyReason: val.moneyReason || '' };
    const i = (o.custom || []).findIndex((c) => c.company === company);
    if (i >= 0) Object.assign(o.custom[i], patch);
    else o.edits[company] = { ...(o.edits[company] || {}), ...patch };
  }
